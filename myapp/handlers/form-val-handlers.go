...
func (h *Handlers) Form(rw http.ResponseWriter, r *http.Request) {
	vars := make(jet.VarMap)
	validator := h.App.Validator(nil)
	vars.Set("validator", validator)
	vars.Set("user", data.User{})

	err := h.render(rw, r, "form", vars, nil)
	if err != nil {
		h.App.ErrorLog.Println(err)
	}
}

func (h *Handlers) PostForm(rw http.ResponseWriter, r *http.Request) {
	err := r.ParseForm()
	if err != nil {
		h.App.ErrorLog.Println(err)
		return
	...
	if !validator.Valid() {
		...
		if err := h.render(rw, r, "form", vars, nil); err != nil {
			h.App.ErrorLog.Println(err)
			return
		}
